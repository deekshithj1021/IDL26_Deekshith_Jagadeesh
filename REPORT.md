...
Early stopping worked and gave good results. But then I understood to keep the code simple and
...
one number using their harmonic mean. The key word is macro, it calculates F1 for each class separately and then
...
stopping at various patience values.
...
|---------|----------|------------|----------|------|
| cells | 90% | ResNet18 | 95.44% | YES |
| chest | 87% | ResNet18 | 88.94% | YES |
| lesions | 67% | ResNet18 | 72.12% | YES |
| orgs | 83% | AlexNet | 88.85% | YES |
...
was the weakest here despite having the most parameters, 12.6 million parameters on a medium-sized dataset of
...
PlantNet from Task 2 actually achieved 90.74% on orgs, the best result of all four models, making it the
...
using the same config.json settings as the original models, batch size 64, 10 epochs, best weight saving active.
...
parameters. This surprised me initially but makes sense, VGG16 uses simple sequential conv layers while ResNet18
...
Think of it like using a truck to deliver a single letter, the tool is far bigger than the task needs.
...
samples VGG16 overfits almost immediately from scratch, it memorizes the 500 training images instead of
...
also adapting to new data. This was an important finding, transfer learning does not automatically help every
architecture.
